const MQPostfixparser = (MQinfixInput: string) => {
  ...
  const lazymath = (word: String) => {
    ...
    for (let i = 0; i < l; i++) {
      ...
      if (a[i]!.localeCompare("(") == 0) {
        if (i - 1 > -1) {
          if (number.test(a[i - 1]!) || alphabet.test(a[i - 1]!))
  ...
  const MQinfixToPostfix = (word: String) => {
    ...
    for (let i = 0; i < l; i++) {
      ...
      if (
        typeof operator[a[i] as keyof typeof operator] != "undefined" ||
        typeof operator[literal as keyof typeof operator] != "undefined"
      ) {
        if (typeof operator[a[i] as keyof typeof operator] != "undefined")
          cOp = operator[a[i] as keyof typeof operator];
        else {
          cOp = literal;
          literal = "";
        }
        ...
        while ("(".localeCompare(stack[stack.length - 1]!) != 0) {
