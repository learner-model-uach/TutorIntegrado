  ...
  const lazymath = (word: String) => {
    ...
    for (let i = 0; i < l; i++) {
      ...
      if (alphabet.test(value)) {
        ...
        if (i - 1 > -1) {
          if (lvalue != undefined && number.test(lvalue))
      ...
      if (value.localeCompare("(") == 0 && flag) {
        if (i - 1 > -1) {
          if (
            lvalue != undefined &&
            (number.test(lvalue) || alphabet.test(lvalue))
          )
            replacePositions.push({ i: i, lit: " " });
        }
      }
      if (value.localeCompare(")") == 0) {
        if (i + 1 < l) {
          if (
            rvalue != undefined &&
            (number.test(rvalue) || alphabet.test(rvalue))
          )
            replacePositions.push({ i: i + 1, lit: " " });
        }
      }
    ...
    for (let i = 0; i < replacePositions.length; i++) {
      ...
      if (lit?.localeCompare(" ") != 0 && lit != undefined) {
        ...
        if (repPos != undefined)
          a = replaceAt(a, repPos.i - llit + acc, repPos.i + acc, lit);
        acc = acc + litAr.length - 1;
      } else {
        if (repPos != undefined)
          a = replaceAt(a, repPos.i + acc, repPos.i + acc, "*");
        acc = acc + 1;
      }
    }
    return a;
      ...
      if (
        ...
        if ("-".localeCompare(cOp) == 0 && l - i > 1) {
          ...
          if (
            ...
            (leftvalue != undefined &&
              i > 0 &&
              (typeof operator[leftvalue as keyof typeof operator] !=
                "undefined" ||
        ...
        while (
          ...
          (precedense[stack[stack.length - 1] as keyof typeof precedense] >
            ...
            (precedense[stack[stack.length - 1] as keyof typeof precedense] ==
              ...
              (typeof associativity[value as keyof typeof associativity] !=
              "undefined"
