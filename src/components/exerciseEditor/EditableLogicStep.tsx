...
const MultiplePlaceholders = dynamic(() => import("../LogicTutor/MultiplePlaceholders"), {
  ssr: false,
});
...
function StepPreview({ step, exerciseCode }) {
  ...
  switch (step.stepType) {
    ...
    case "MultiplePlaceholders":
      return <MultiplePlaceholders {...props} />;
    case "SinglePlaceholder":
      return <SinglePlaceholder {...props} />;
    case "TableStep":
...
export default function EditableLogicStep({ step, stepName, setSteps, exerciseCode }) {
  const safeStep = step ?? {};
  ...
  const isEditing =
    isEditingStep || isEditingMessages || isEditingAnswers || isEditingHints || isEditingChoices;
  ...
  return (
    <Box borderWidth="2px" borderColor={formBg} borderRadius="lg" p={4} mb={4}>
      <Heading as="h2" textAlign="center" mb={4} fontSize="lg">
        {stepName}{" "}
        <Badge ml={2} colorPalette="purple" fontSize="xs">
          {stepType}
        </Badge>
      </Heading>
      ...
      {isEditingStep && (
        <Box>
          ...
          <Box bg={formBg} borderRadius="md" p={4} mt={2}>
            ...
            <Field.Root>
              <Field.Label>stepType</Field.Label>
              <Input
                value={localStepCopy.stepType || ""}
                onChange={e => handleField("stepType", e.target.value)}
                placeholder="Alternatives | TrueFalse | Blank | InputButtons | MultiplePlaceholders | SinglePlaceholder | TableStep"
              />
            </Field.Root>
          </Box>
        </Box>
      ...
      {hasChoices && (
        <>
          <EditButton
            width="full"
            isEditing={isEditingChoices}
            mt={3}
            onClick={() => {
              if (isEditingChoices) setLocalStepCopy({ ...localStep });
              setIsEditingChoices(!isEditingChoices);
            }}
                ...
                {localStepCopy.multipleChoice.map((choice, i) => (
                  <Box key={i} mb={4} p={3} bg="white" borderRadius="md">
                    ...
                    <Field.Root mb={2}>
                      ...
                      <Input
                        value={choice.text || ""}
                        onChange={e => handleChoice(i, "text", e.target.value)}
                      />
                    </Field.Root>
                    <Field.Root mb={2}>
                      <Field.Label>Expresión (LaTeX)</Field.Label>
                      <Input
                        ...
                        onChange={e => handleChoice(i, "expression", e.target.value)}
                      />
                    </Field.Root>
                    <Field.Root>
                      <Field.Label>¿Correcto? (true/false)</Field.Label>
                      <Input
                        value={String(choice.correct)}
                        onChange={e => handleChoice(i, "correct", e.target.value === "true")}
                      />
                    </Field.Root>
                  </Box>
                ))}
          ...
          <Box bg={formBg} borderRadius="md" p={4} mt={2}>
            ...
            {localStepCopy.answers.map((answer, i) => (
              <Field.Root key={i} mb={3}>
                <Field.Label>answers[{i}].nextStep</Field.Label>
                <Input
                  value={answer.nextStep ?? ""}
                  onChange={e => handleAnswer(i, "nextStep", e.target.value)}
                  placeholder="-1 para finalizar"
                />
              </Field.Root>
            ))}
          </Box>
          ...
          <Box bg={formBg} borderRadius="md" p={4} mt={2}>
            {localStepCopy.hints.map((hint, i) => (
              <Field.Root key={i} mb={3}>
                ...
                <Input value={hint.hint || ""} onChange={e => handleHint(i, e.target.value)} />
              </Field.Root>
            ))}
          </Box>
        </Box>
