...
const MathQuillBase = dynamic(
  () => import("react-mathquill").then((mod) => mod.EditableMathField),
  {
    ssr: false,
    loading: () => (
      <Box height="40px" bg="bg.muted" borderRadius="md" className="mq-loading" />
    ),
  }
);
...
export const EditableMathField = React.forwardRef<
  HTMLDivElement,
...
>(({ latex, onChange, config, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      {isMounted && (
        <MathQuillBase latex={latex} onChange={onChange} config={config} />
      )}
    </Box>
  );
});
