...
const FCstep1 = dynamic(() => import("../tutorFactorizacion/factorComun/steps/FCstep1"), {
  ssr: false,
});
const FCCstep1 = dynamic(
  () => import("../tutorFactorizacion/factorComunCompuesto/steps/FCCstep1"),
  { ssr: false },
);
const FCCstep2 = dynamic(
  () => import("../tutorFactorizacion/factorComunCompuesto/steps/FCCstep2"),
  { ssr: false },
);
const DCstep1 = dynamic(
  () =>
    import("../tutorFactorizacion/diferenciaCuadrados/steps/DCstep1").then(m => ({
      default: m.DCstep1,
    })),
  { ssr: false },
);
const DCstep2 = dynamic(
  () =>
    import("../tutorFactorizacion/diferenciaCuadrados/steps/DCstep2").then(m => ({
      default: m.DCstep2,
    })),
  { ssr: false },
);
const DSCstep1 = dynamic(
  () =>
    import("../tutorFactorizacion/diferenciaSumaCubos/steps/DSCstep1").then(m => ({
      default: m.DSCstep1,
    })),
  { ssr: false },
);
const DSCstep2 = dynamic(
  () =>
    import("../tutorFactorizacion/diferenciaSumaCubos/steps/DSCstep2").then(m => ({
      default: m.DSCstep2,
    })),
  { ssr: false },
);
const TCstep1 = dynamic(
  () =>
    import("../tutorFactorizacion/trinomiosCuadraticos/steps/TCstep1").then(m => ({
      default: m.TCstep1,
    })),
  { ssr: false },
);
const TCstep2 = dynamic(
  () =>
    import("../tutorFactorizacion/trinomiosCuadraticos/steps/TCstep2").then(m => ({
      default: m.TCstep2,
    })),
  { ssr: false },
);
const TCstep3 = dynamic(
  () =>
    import("../tutorFactorizacion/trinomiosCuadraticos/steps/TCstep3").then(m => ({
      default: m.TCstep3,
    })),
  { ssr: false },
);
const TCstep4 = dynamic(
  () =>
    import("../tutorFactorizacion/trinomiosCuadraticos/steps/TCstep4").then(m => ({
      default: m.TCstep4,
    })),
  { ssr: false },
);
const TCstep5 = dynamic(
  () =>
    import("../tutorFactorizacion/trinomiosCuadraticos/steps/TCstep5").then(m => ({
      default: m.TCstep5,
    })),
  { ssr: false },
);
...
function StepPreview({ step, exerciseType, stepIndex, exerciseCode, topicId }) {
  if (!step) return null;
  const common = {
    contentID: exerciseCode ?? "preview",
    topicID: topicId ?? "",
    isEditorMode: true,
  };
...
export default function EditableFacStep({
  step,
  index,
  stepName,
  setSteps,
      ...
      {isEditingStep && (
        <Box>
          ...
          <Box bg={formBg} borderRadius="md" p={4} mt={2}>
            ...
            <Field.Root>
              <Field.Label>displayResult</Field.Label>
              <Input
                value={localStepCopy.displayResult || ""}
                onChange={e => handleField("displayResult", e.target.value)}
              />
            </Field.Root>
          </Box>
        </Box>
          ...
          <Box bg={formBg} borderRadius="md" p={4} mt={2}>
            {localStepCopy.answers.map((answer, i) => (
              <Field.Root key={i} mb={3}>
                <Field.Label>Respuesta {i + 1}</Field.Label>
                <Input
                  value={
        ...
        <Box>
          ...
          <Box bg={formBg} borderRadius="md" p={4} mt={2}>
            {localStepCopy.hints.map((hint, i) => (
              <Box key={i} mb={4}>
                <Field.Root mb={2}>
                  <Field.Label>Pista {i + 1} — texto</Field.Label>
                  <Input
                    value={hint.hint || ""}
                ...
                {hint.expression !== undefined && (
                  <Field.Root>
                    <Field.Label>Pista {i + 1} — expresión</Field.Label>
                    <Input
                      value={hint.expression || ""}
                      onChange={e => handleHint(i, "expression", e.target.value)}
                    />
                  </Field.Root>
                )}
              </Box>
