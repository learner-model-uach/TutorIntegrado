...
const Enabledhint = ({
  ...
  useEffect(() => {
    if (!isEditorMode) MQProxy.error = error;
  }, [error]);
  useEffect(() => {
    setError(mqSnap.error);
  }, [mqSnap.error]);
  useEffect(() => {
    if (!isEditorMode) MQProxy.hints = hints;
  }, [hints]);
...
const Mq2 = ({
  ...
  const handleAnswer = () => {
    ...
    if (correctAns) {
      result.current = true;
      if (!isEditorMode) {
        // ✅ solo actualiza MQProxy en modo estudiante
  ...
  const refMQElement = (mathquill: MathField) => {
    if (ta == undefined) setTa(mathquill);
  };
        ...
        <Box>
          <Stack gap={4} direction="row" align="center" pb={4}>
            {[
              ...
              >
                {cmd.startsWith("\\") ? (
                  <MQStaticMathField exp={label} currentExpIndex={false} />
                ) : (
                  label
                )}
              </Button>
