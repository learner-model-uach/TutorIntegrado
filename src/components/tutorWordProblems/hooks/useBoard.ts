...
export const useBoard = (boardId: string, graphSettings: settings) =>{
  ...
  const maxBoundingBox = graphSettings?.maxBounding ? [graphSettings.maxBounding.X1, graphSettings.maxBounding.Y1, graphSettings.maxBounding.X2, graphSettings.maxBounding.Y2] as [number, number, number,number] : [-Infinity, Infinity, Infinity, -Infinity] as [number, number, number,number] 
  ...
  useEffect(()=>{
    const board = JXG.JSXGraph.initBoard(boardId,{
      ...
      pan: { // establece el compontamiento en dispositivos mobiles, requiere doble touch en el grafico para moverse dentro
        enabled: true,
        needTwoFingers: true
      },
      browserPan: true ,
    ...
    if (!showOriginAxis && xAxisSettings && yAxisSettings){
        ...
        const xAxis = board.create("axis", [xAxisSettings?.point1, xAxisSettings?.point2], { // punto 1 (x,y) , punto 2 (x,y)
          ticks: {
            drawLabels: true,
