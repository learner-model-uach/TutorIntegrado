...
export const useHint = (hints: Hint[]) =>{
  ...
  const  nextHint = ()=>{currentHint< (totalHints-1) && setCurrentHint(currentHint+1)}

  const prevHint = () =>{currentHint>0 && setCurrentHint(currentHint-1)}
  ...
  const resetNumHintsActivated = () => {setNumHintsActivated(0);};
  return{ unlockedHints, currentHint, totalHints,disabledPrevButton,disabledNextButton,numHintsActivated,nextHint,prevHint,
    unlockHint,resetNumHintsActivated,
  }
}