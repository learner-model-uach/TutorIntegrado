...
import { ACCORDION_COLOR, CORRECT_BUTTOM_NAME, CORRECT_ANSWER_COLOR, INCORRECT_ANSWER_COLOR } from "../types";
import { useAction } from "../../../utils/action";

export const StepInput = ({
  step, setNumStep, nStep, setDisableState, totalSteps, setStepCorrect,
  setColor, setNextExercise, code, topicId, updateObjectSteps, completeContentSteps,
  ...
  const getHints = answerId => {
    ...
    if (hintsStep != undefined) {
      ...
      if (filterHint != undefined) {
        filterHint = filterHint.concat(hintsStep.filter(hint => hint.generic && !filterHint.includes(hint)));
      } else {
  ...
  const checkLastStep = () => {
    if (nStep == totalSteps - 1) {
      startAction({ verbName: "completeContent", contentID: code, topicID: topicId,
        result: Number(isCorrect), extra: { steps: completeContentSteps } });
  ...
  const checkAnswers = e => {
    ...
    if (step.stepId === nStep.toString()) {
      if (idUserAnswer === step.correct_answer) {
        startAction({ verbName: "tryStep", contentID: code, topicID: topicId,
          stepID: step.stepId, result: 1, kcsIDs: step.KCs,
          extra: { response: answer, attemps: attempts, hints: hintsShow } });
        if (!isEditorMode) {
          setStepCorrect(state => [...state, answer]);
          setColor(prev => [...prev.slice(0, nStep), CORRECT_ANSWER_COLOR, ...prev.slice(nStep + 1)]);
        ...
        startAction({ verbName: "tryStep", contentID: code, topicID: topicId,
          stepID: step.stepId, result: 0, kcsIDs: step.KCs,
          extra: { response: answer, attemps: attempts, hints: hintsShow } });
      }
    }
  };
  ...
  return (
    <Stack>
      <Stack>
        <VStack direction={["row", "column"]} style={{ borderRadius: 10, padding: 10, width: "100%" }}>
          <Stack direction={{ base: ["row", "column"], xl: ["column", "row"] }}>
            <Flex marginRight={{ xl: "250px" }} margin={{ base: "auto" }}>
              <Text display={{ base: "none", xl: "block" }} margin={{ base: "auto" }}>{step.left_text}</Text>
              <Flex padding={{ base: "5px" }}>
                {step.input_labels && <TeX style={{ fontSize: "12px", margin: "auto" }} math={step.input_labels} as="figcaption" />}
              </Flex>
            ...
            <Stack marginLeft={{ base: "0px", xl: "-180px" }}>
              <Grid display={{ xl: "none", base: "grid" }} style={{ margin: "10px" }} templateColumns="repeat(2, 1fr)" gap={6}>
                <Button colorPalette="blue" onClick={checkAnswers}>{CORRECT_BUTTOM_NAME}</Button>
                <Hint firstTimeHint={firstTimeHint} hints={hints} setNewHintAvaliable={setNewHintAvaliable}
                  answerId={idAnswer} newHintAvaliable={newHintAvaliable} code={code} nStep={nStep} setHintsShow={setHintsShow} />
              </Grid>
